...
int go_to(Inputs inputs)
{
 	...
 	while(flg == 0)
	{
		...
		else if(selection == 5)
		{
			...
			DistTheta = reactive_students(observations, quantized_attraction, quantized_intensity,inputs.Mag_Advance,inputs.max_angle, inputs.num_sensors, angle_light, coord_robot);
