...
int go_to(Inputs inputs)
{
 	...
 	while(flg == 0)
	{
		...
		else if(selection == 6)
		{
			...
			DistTheta = state_machine_students(observations, quantized_attraction, quantized_intensity,state,&next_state,inputs.Mag_Advance,inputs.max_angle, inputs.num_sensors, coord_robot, coord_dest, angle_light);
			#ifdef DEBUG
