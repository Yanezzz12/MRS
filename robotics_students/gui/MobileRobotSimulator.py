	...
	def grasp_object(self,name):
		...
		if self.grasp_id==False :
			if self.exist_object(name) :
				...
				dist = math.sqrt(((self.robotX*self.mapX)/self.canvasX-obj[1])**2+(((self.canvasY-self.robotY)*self.mapY)/self.canvasY-obj[2])**2 )
				#if  (dist = math.sqrt(((self.robotX*self.mapX)/self.canvasX-obj[1])**2+(((self.canvasY-self.robotY)*self.mapY)/self.canvasY-obj[2])**2 )) < distancia_min:
				if  dist < distancia_min:
					self.grasp_id = name
					self.w.delete(obj[3])
					self.w.delete(obj[4])
					print("Distance to object "+name+" is "+str(dist))
					return True
				else:
					print("Distance to object "+name+" is "+str(dist)+" more than "+str(distancia_min))
			else:
